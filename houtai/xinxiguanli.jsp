...
<script>
    var $table = $('#table');
    var servlet_url = "/mangers/gongqiuguanli";
    $(function() {
        ...
        $table.bootstrapTable({
            ...
            columns: [
                ...
                {field: 'title', title: '信息标题',halign: 'center',align:"center"},
               {field: 'desc', title: '信息描述', halign: 'center',align:"center"},
                ...
                {field:'uid',title:'发布人',halign: 'center',align:"center"},
                {field:'sontype',title:'二级ID',halign: 'center',align:"center"},
                {field:'ftype',title:'一级ID',halign:'center',align:"center"},
                {field:'status',title:'发布状态',halign:'center',align:'center',formatter:'statusFormatter'},
                {field:'gqid',title:'供/求',halign:'cneter',align:'center',formatter:'gqidFormatter'},
    ...
    function gqidFormatter(value,row,index){
        if (row.gqid==1){
            return "求购";
        }else if (row.gqid==2){
            return "供应";
            ...
            $.confirm({
                ...
                buttons:{
                    confirm:{
                        ...
                        action:function () {
                            //执行删除操作
                            $.post(servlet_url+"?m=delete",{'id':row.id,'title':row.title,'uid':row.uid,'gqid':row.gqid,'status':row.status},function (txt) {
                                $.alert(txt.message);
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function () {
                            ...
                            var gqid=rows[0].gqid;
                            var status = $("#zhuangtai").val();
                            $.post(servlet_url+'?m=shenhe',{"title":title,"id":id,"gqid":gqid,"uid":uid,"status":status},function(txt){
                                $.alert(txt.message)
                                $table.bootstrapTable('refresh');
                            },"JSON")
